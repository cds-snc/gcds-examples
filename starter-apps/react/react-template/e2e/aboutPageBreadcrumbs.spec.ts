...
test('shows localized breadcrumbs for about topic route', async ({ page }) => {
  ...
  const enBreadcrumbs = await page.locator('gcds-breadcrumbs gcds-breadcrumbs-item').allTextContents()
  ...
  const frBreadcrumbs = await page.locator('gcds-breadcrumbs gcds-breadcrumbs-item').allTextContents()
